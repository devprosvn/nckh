...
class CarlaEnv(gym.Env):
        ...
        while reset_attempts < max_reset_attempts:
            try:
                ...
                while ego_spawn_times <= self.max_ego_spawn_times:
                    ...
                    time.sleep(0.5)
                if self.ego is None:
                    self.logger.error("Failed to spawn ego vehicle after %d attempts", self.max_ego_spawn_times)
                ...
                init_speed = carla.Vector3D(x=self.desired_speed * np.cos(yaw), y=self.desired_speed * np.sin(yaw))
                ...
                self.state_info['lateral_dist_t'] = np.linalg.norm(pos_err_vec) * \
                    np.sign(pos_err_vec[0] * road_heading[1] - pos_err_vec[1] * road_heading[0])
                ...
                self.logger.error("Env reset() error: %s", str(e))
                time.sleep(2)
                self._make_carla_client('localhost', self.port)
                reset_attempts += 1
        raise RuntimeError("Failed to reset environment after %d attempts" % max_reset_attempts)
    ...
    def _create_vehicle_bluepprint(self, actor_filter, color=None, number_of_wheels=[4]):
        ...
        if not blueprint_library:
            self.logger.error("No blueprints with %d wheels for filter %s", number_of_wheels, actor_filter)
            raise RuntimeError("No valid blueprints found")
    ...
    def _try_spawn_ego_vehicle_at(self, transform):
        self.logger.info("Attempting to spawn vehicle at transform: %s", transform)
        vehicle = self.world.spawn_actor(self.ego_bp, transform)
        if vehicle is not None:
            self.actors.append(vehicle)
            self.ego = vehicle
            self.logger.info("Spawned vehicle with type_id: %s", vehicle.type_id)
            return True
        self.logger.warning("Failed to spawn vehicle at transform: %s", transform)
        return False
    ...
    def _make_carla_client(self, host, port):
        ...
        for attempt in range(max_attempts):
            try:
                self.logger.info("Connecting to Carla server at %s:%d (attempt %d/%d)", host, port, attempt + 1, max_attempts)
                self.client = carla.Client(host, port)
                self.client.set_timeout(20.0)
                self.world = self.client.load_world('Town05')
                self.map = self.world.get_map()
                blueprints = self.world.get_blueprint_library().filter('vehicle.lincoln*')
                self.logger.info("Available vehicle.lincoln* blueprints: %s", [bp.id for bp in blueprints])
                if not blueprints:
                    raise RuntimeError("No vehicle.lincoln* blueprints available")
                self.world.set_weather(carla.WeatherParameters.ClearNoon)
                self.logger.info("Carla server connected on port %d", port)
                return
            except Exception as e:
                self.logger.error("Failed to connect to Carla server: %s", str(e))
                time.sleep(2)
        raise RuntimeError("Failed to connect to Carla server after %d attempts" % max_attempts)
